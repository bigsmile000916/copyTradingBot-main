    ...
    for day in (start_date + timedelta(n) for n in range(NUM)):
        if (not no_weekends or day.weekday() < 5) \
            ...
            for commit_time in (day + timedelta(minutes=m)
                ...
                if randint(1, 10) is 3:             # add by vector 0714  frequency
...
def arguments(argsval):
    ...
    parser.add_argument('-un', '--user_name', type=str, required=False, default="BritnessDev",
                        help="""Overrides user.name git config.
                        If not specified, the global config is used.""")
    parser.add_argument('-ue', '--user_email', type=str, required=False, default="britness.gmd@gmail.com",
